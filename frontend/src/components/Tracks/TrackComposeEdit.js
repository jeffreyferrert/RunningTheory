...
import { clearTrackErrors, composeTrack, editTrack, fetchTracks } from '../../store/tracks';
...
function TrackCompose() {
  ...
  const filteredTracks = tracks.filter(track => track._id === trackId);

  useEffect(() => {
    dispatch(fetchTracks());
    setName(filteredTracks[0].name)
    setLocation(filteredTracks[0].location)
    setDescription(filteredTracks[0].description)
    setStartAddress(filteredTracks[0].startAddress)
    setEndAddress(filteredTracks[0].endAddress)
    setBicycleDistance(filteredTracks[0].miles)
  }, [dispatch])
  ...
  const handleSubmit = e => {
    e.preventDefault();
    dispatch(editTrack(filteredTracks[0]._id, { name, location, miles: bicycleDistance, description, startAddress, endAddress })).then(() => history.push(`/tracks/${trackId}`))
  };
